...
class _CardCaptureScreenState extends ConsumerState<CardCaptureScreen>
    ...
    if (widget.bereal) {
      _berealTimer = Timer.periodic(const Duration(seconds: 1), (_) {
        ...
        if (_berealRemaining <= 0) {
          _berealTimer?.cancel();
          // ⚠️ Le messager est saisi AVANT la sortie : après, le contexte de
          // cet écran est en train d'être démonté, et le chercher depuis lui
          // revient à interroger un arbre qu'on vient de quitter.
  ...
  /// **Cet écran a-t-il déjà demandé à partir ?**
  ///
  /// Un verrou, pas un délai — la nuance est tout : il est porté par CET écran,
  /// naît avec lui et meurt avec lui. Deux fermetures rapides sur deux écrans
  /// différents ne se voient donc jamais ; seules deux sorties **du même
  /// écran** s'annulent, ce qui est précisément ce qu'on veut.
  var _leaving = false;

  /// La sortie de l'écran de capture — **le seul endroit qui appelle `pop`**.
  ///
  /// ### Le défaut que ça supprime
  ///
  /// La croix et le minuteur BeReal appelaient chacun `Navigator.pop()` sans
  /// se connaître. Deux `pop` qui s'enchaînent retirent **deux** écrans : la
  /// caméra, puis celui du dessous. Selon la plateforme on se retrouve alors
  /// sur un écran vide ou hors de l'app — sans erreur, sans trace. Un défaut
  /// qui ne laisse rien derrière lui coûte plus cher qu'un plantage.
  ///
  /// La course existait **avant** le glissement de sortie (croix + minuteur).
  /// Elle devenait simplement plus facile à rencontrer avec une troisième
  /// sortie.
  ///
  /// ⚠️ Toute nouvelle façon de quitter cet écran doit passer par ici.
  void _leave() {
  ...
  /// 2026-08-16). Même destination que la croix : on rend la main à l'écran
  /// d'où l'on vient, quel qu'il soit.
  ///
  /// ### Pourquoi une ENVELOPPE et pas une bande posée par-dessus
  ///
  /// Le sélecteur de type est un `PageView` qui occupe toute la largeur : une
  /// bande posée au-dessus lui volerait son geste (dans un `Stack`, le test de
  /// contact part du dernier enfant). En enveloppe, il est **enfant** — et
  /// l'arène de gestes donne toujours la priorité à l'enfant. Aucune zone n'est
  /// codée en dur : une géométrie écrite à la main devient fausse au premier
  /// changement de mise en page, **et fausse en silence**.
  ///
  /// ### Ce qui est volontairement exclu
  ///
  /// - **Le BeReal** (décision de Jay) ;
  /// - **tout ce qui n'est pas la vue caméra nue** : enregistrement en cours,
  ///   verso, récapitulatif, traitement. Le glissement est un raccourci pour
  ///   *quitter la caméra*, pas pour *abandonner une prise en cours* — et il
  ///   est bien plus facile à déclencher par mégarde qu'une croix en haut à
  ///   gauche.
  bool get _edgeExitAllowed =>
      !widget.bereal && !_recording && !_busy && _step == 0;
  ...
  void _onExitDragEnd(DragEndDetails details) {
    if (!_fromRightEdge) return;
    _fromRightEdge = false;
    // Vers l'INTÉRIEUR de l'écran, comme le retour d'Android : le geste part du
    // bord et ramène vers le centre. Le sens est celui que la main connaît
    // déjà, pas un sens de plus à apprendre.
    if ((details.primaryVelocity ?? 0) <= -_exitVelocity) _leave();
  }

  /// Assez franc pour ne pas partir sur une hésitation, assez bas pour ne pas
  /// exiger un geste sec.
  static const _exitVelocity = 320.0;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      // Le glissement de sortie ENVELOPPE l'écran : tout ce qui réclame un
      // glissement horizontal à l'intérieur (le sélecteur de type, la palette)
      // reste prioritaire, parce que l'arène de gestes retient toujours
      // l'enfant. Voir `_onExitDragStart`.
      //
