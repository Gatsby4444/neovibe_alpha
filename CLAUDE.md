...
- **UN SEUL FORMAT : LA VIBE** (Jay, 2026-09-21, après un pré-mortem : *« NeoVibe doit être cool et être l'app qu'on sort en soirée […] pas l'app qui permet différentes publications avec des formats différents et mélangés »*). Les **albums** (« publications ») et les **Flows**, construits du 2026-09-15 au 2026-09-20, sont **retirés du MVP et du code** ; **un éditeur pour un format** ; **Pulse n'affiche que des Vibes** (le feed reste, avec ses trois sources et ses trois modes). L'acquis est conservé par le tag `v0.9.236` et décrit dans **`docs/formats-mis-de-cote.md`** (où le retrouver, comment le réintroduire) ; le moteur d'édition est resté sous l'éditeur de Vibes (`lib/features/cards/editor/`). La base garde son schéma ; l'app n'écrit et ne lit que `kind = 'card'`. ⚠️ Ne pas réintroduire un format sans décision de Jay — la question reste *« ce contenu a-t-il obligé quelqu'un à sortir de chez lui ? »*. À venir, sur la Vibe elle-même : la rendre plus polyvalente (ex. vidéos importées).
- **La publication est une file NATIVE et persistante** (Jay, 2026-09-19 : *« faire comme les autres, les 5 points, même le 5ᵉ car c'est essentiel »*) : le Dart dépose une tâche et affiche son état ; un service Kotlin scelle, envoie par morceaux reprenables et enregistre, avec une notification, et survit à la fermeture de l'app. La publication apparaît tout de suite dans la grille avec sa progression. ✅ **Construit le 2026-09-19** (v0.9.220) pour les albums ; **les Vibes y passent depuis le 2026-09-21** (Jay : *« on fait la file native maintenant pour les vibes »*) — `LibraryRepository.publish` dépose et libère aussitôt, les faces arrivent finales (rendues par l'éditeur avant l'envoi), le service remet l'index MP4 en tête puis scelle. `android/…/publish/`, `lib/core/publish/`, `PendingCell` — description dans `docs/file-de-publication.md`. Corollaire de rangement : le dossier d'une publication est `<filesDir>/publish/<id>/`, **jamais sous `work/`** (balayé au démarrage) ; `job.json` n'est écrit que par le service, `release.json` et `cancel` que par l'app — deux écrivains, deux fichiers.
- **Une Vibe ne se garde JAMAIS à l'insu de celui qui l'a prise** (Jay, 2026-09-20). Les brouillons (Réglages › Brouillons, 3 jours) enregistraient **d'eux-mêmes** les publications et les Flows — des imports de la galerie, sortis du MVP le 2026-09-21 — mais une **Vibe**, format façon Snap souvent pris à la caméra pour un envoi privé, n'est écrite sur le disque **que si l'utilisateur choisit « Garder en brouillon »** dans la popup de sortie. Sans ce choix (app tuée, téléphone éteint, « Supprimer »), ses fichiers sont effacés au prochain démarrage. `VibeDraftKeeper` n'écrit que sur `flush()`, jamais seul.
...
- **La symétrie du croisement est absolue** (2026-08-20, **reconfirmée par Jay le 2026-09-21** face aux idées de « contact anonyme vers un inconnu détecté ») : *« on demande toujours à chaque appareil de certifier qu'il voit l'autre avant de créer une connexion »*. Aucun contact, aucune suggestion, aucune découverte asymétrique. Ce qui compte les présents ne le fait **que dans un événement** (le lieu filtre), jamais dans la rue.
- **La mémoire des rencontres est acceptée** (Jay, 2026-09-21) : NeoVibe garde qui on a rencontré, où et quand — un journal assumé, avec ses durées en base (à trancher : suggestion 3 jours, rencontre 1 an) et effaçable par l'utilisateur. Direction associée : **la super galerie** (les moments en albums : où, quand, avec qui ; les événements, avec quels amis) et **la bibliothèque partagée automatique** quand des amis sont ensemble. Textes et liste complète : `docs/raison-d-entrer-2026-09-21.md`, RAPPELS #155.
- **Une plateforme d'administration et de modération complète avant la première release de production** (Jay, 2026-09-21) — condition, pas option. RAPPELS #156.
