...
| # | Sujet | Détail | Depuis |
|---|-------|--------|--------|
| 2 | **Aperçu à l'envers pendant la bascule de caméra** (mineur, connu depuis le début) | En Mono et en Oneshot séquentiel, la bascule avant/arrière montre brièvement l'image **renversée**, puis un petit sursaut de correction. Entre l'arrière (capteur 90°) et l'avant (270°) il y a 180° d'écart. **Tentative en v0.9.24 : attendre l'événement `previewInfo` de la nouvelle caméra avant de dévoiler → N'A PAS SUFFI** (testé par Jay : « quelque chose a changé mais le problème est toujours là »). **Hypothèse à vérifier ensuite** : `previewInfo` arrive à la configuration de session, donc AVANT les premières images ; la nouvelle rotation est alors appliquée aux **dernières images de la caméra précédente**, encore dans la texture — ce qui produit exactement le même symptôme. Piste : attendre un vrai signal de « première image de la nouvelle caméra » (côté natif, `SurfaceProducer`/`onFrameAvailable`) plutôt que l'info de transformation. Jay : problème mineur, à revoir plus tard, ne pas y consacrer de temps maintenant. | 2026-07-25 |
