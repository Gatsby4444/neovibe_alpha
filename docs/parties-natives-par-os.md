...
| **Gestes de la carte** *(2026-09-26)* | `map/MapGestureTuner.kt` (canal `neovibe/map_gestures`, méthode `tune` → `{trouvees, reglees}`) + `res/values/mapbox_gestures.xml` | Règle les SEUILS du moteur de gestes de Mapbox pour la hiérarchie de Google Maps relevée par Jay. Ressources redéfinies (priorité de l'app à l'assemblage, vérifiée sur l'APK) : inclinaison après 20dp, zoom après 16dp, zoom pendant une rotation après 24dp. Écrits en dur, réglés par le code : angle de départ de rotation 10°, angle max des doigts pour incliner 70°, inclinaison ×2 (écouteur `OnShoveListener` qui ajoute la part manquante). **Élan** : celui de Mapbox (seulement au-delà de 1 000 dp/s) est coupé côté Dart, remplacé par un `OverScroller` d'Android (vitesse mesurée par `VelocityTracker`, doigt levé compris ; pas via `cameraForDrag` à chaque image ; arrêté par un doigt posé) après un déplacement à un doigt seulement. **Journal des gestes** (méthode `journal`) : 40 derniers gestes, détecteurs partis dans l'ordre, doigts, écart, angle, élan — section « CARTE — LES GESTES » du diagnostic. Le toucher est observé par `setOnTouchListener` (renvoie `false`). **Arbitrage à deux doigts** (2026-09-26) : zoom/rotation/inclinaison retenus au 2e doigt, le mouvement dominant après 10dp libère le sien — calcul pur dans `map/TwoFingerArbiter.kt` (glissement commun ; écart et rotation relatifs, trois parts dans la même unité, rotation devant le zoom à 5 % près), testé sur la JVM (`TwoFingerArbiterTest`, 11 cas). *(Une « règle des deux doigts » a été essayée en v0.9.290 puis retirée à la demande de Jay.)* Mode d'affichage FIXE : couche de texture (`TLHC_HC`), choisi par Jay ; la carte est trouvée dans les vues de l'écran (elle ne le serait pas en écran virtuel). **Images mesurées** pendant chaque geste (`subscribeRenderFrameFinished`). **Élan** signalé comme geste (`setGestureInProgress`). **Verrou** (2026-09-26) : dès qu'un zoom à deux doigts, une rotation ou une inclinaison part, le détecteur de déplacement est coupé jusqu'au prochain premier doigt posé (recoupé après chaque fin de geste, Mapbox le rendant dans `handleShoveEnd`). La vue native est cherchée dans les vues de l'activité (le mode d'affichage fixé — couche de texture — l'y accroche). Compilé contre `com.mapbox.maps:android-ndk27:11.31.1` en `compileOnly` (fourni par le paquet). iOS : `MapView.gestures.options` du SDK iOS (seuils à retrouver) |

| **Cadence de l'écran** *(2026-09-26)* | `DisplayRate.kt` (canal `neovibe/display`, méthode `etat`) | Au lancement, demande pour la fenêtre le mode d'écran le plus rapide à la même définition (`preferredDisplayModeId`) : sur les Xiaomi, une app qui ne demande rien peut rester à 60 Hz sur un écran 120 Hz. `etat` rend les modes possibles, la cadence au lancement, ce qui a été demandé et la cadence réelle — section « ÉCRAN — CADENCE » du diagnostic. iOS : `CADisplayLink.preferredFrameRateRange` + `CADisableMinimumFrameDurationOnPhone` |
