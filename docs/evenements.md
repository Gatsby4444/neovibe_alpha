...
| **Le Drop se garde côté serveur** (2026-09-25) | `add_vibe_to_library` refuse le Drop d'un événement **fermé** ou **quitté** (présence en cours exigée), et toute Vibe dont l'app ne déclare pas les faces **prises à la caméra** (`p_camera_only` — ni import, ni fond uni ; déclaré, pas vu : RAPPELS #169). Migration `20260925080000_le_drop_se_garde_cote_serveur.sql`. Côté app, le chat d'un événement ne se choisit que depuis ce chat ; partout ailleurs, seul le Drop de l'événement **où je suis** est proposé |
| **Le radar en cartes : affiche, description, amis** (2026-09-25) | L'organisateur pose une **affiche 3:4** et une **description** (≤ 200) à la création ou dans les réglages (`set_event_details`, organisateur et soirée en cours ; coffre `event_posters`, NON chiffré, lecture = qui voit la soirée et seulement l'affiche courante). `my_events` / `nearby_events` rendent `description`, `poster_path` et **`friends_present`** (MES amis présents, jamais un inconnu — décision de Jay). Radar : ma photo s'enfonce dans le fond, puis **le paquet** (`event_deck.dart`) : une carte par soirée, de la plus proche à la plus loin, couches à vitesses différentes, fond teinté par l'affiche, bouton qui suit la carte. Signaler une soirée (`report_event`, affiche sous scellé) ; la console retire l'affiche (`admin_remove_event_poster`). Migration `20260925190000_l_affiche_de_la_soiree.sql`. Tests : `test/event_finder_test.dart` |
...
| **Ma galerie** (sur le téléphone) — refondue le 2026-09-25 | **Des Vibes datées et situées**, plus des albums (Jay). Montre : les Vibes que j'ai **enregistrées moi-même**, et celles des **Drops d'événement où j'étais, sauf les éphémères** — dans un Drop d'événement, « sauvegardable » n'est plus une option, il suit « éphémère » (serveur : `add_vibe_to_library` / `update_drop_vibe`). `GalleryKeeper` les garde à la fermeture avec date, lieu nommé et soirée ; `SavedItem` porte `takenAt` / `city` / `placeName` / `eventId` / `eventTitle` ; `GalleryScreen` : Tout · Soirées, retrouver un jour, retrouver un lieu ; remplace l'écran « Enregistrements ». **Le lieu de prise** est toujours noté, sur le serveur, lisible de son seul auteur (`capture_places`, décision de Jay) ; la **ville** se trouve sur le téléphone (`CityIndex`, GeoNames, rien n'est envoyé à un tiers). **Le nom du lieu** d'un événement est posé par son créateur (`events.place_name`, `set_event_place`, champ « Nom du lieu » à la création) et figure sur les Vibes du Drop. **Le récap** : Événements › Historique (`event_history_screen.dart` : `EventHistoryScreen`, `MomentScreen`). Tests : `test/gallery_test.dart`, `test/city_index_test.dart` |

| **La carte** (étape 5, v0.9.240) | `events_map_screen.dart` : les soirées à portée (épingles « nom · N »), ma position, et — pour l'événement où je suis — ses points chauds en cercles. Fond **Mapbox** depuis le 2026-09-25 (`mapbox_maps_flutter`, style Standard, nuit si l'app est sombre, sans commerces ni arrêts ; jeton public du compte de Jay dans `Env.mapboxToken`) — il remplace OpenStreetMap, interdit pour une vraie app (RAPPELS #157). Ma position vient de `LivePosition`, jamais du point bleu de Mapbox (un chemin, une donnée) ; le halo d'incertitude est un polygone en mètres. Entrées : icône carte de l'écran Événements, puce « Carte » des points chauds |
